...
class unet(object):
    ...
    def ml2e_head(self, target,prediction):
        ...
        if self.save_all_test_error:
            self.save_data_dir = './saved_data/'
            try:
                all_test_error = pickle.load(open(os.path.join(self.save_data_dir, 'all_test_error_ml2e_Ntest_out_M_in_normJ_head.pkl'),'rb'))
            except:
                all_test_error=[]
            all_test_error.append(error_batch_sample)
            pickle.dump(all_test_error, open(os.path.join(self.save_data_dir, 'all_test_error_ml2e_Ntest_out_M_in_normJ_head.pkl'), 'wb'))
    ...
    def unet_3d(self):
        ...
        if self.num_gpu <= 1:
            ...
            print('[INFO] training with {} GPUs ...'.format(self.num_gpu))
